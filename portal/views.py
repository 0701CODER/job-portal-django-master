...
def home_page(request):
    ...
    for i in jobs:
        ...
        if get_days.days > 1:
            i.created_date = f'{get_days.days} the other day'
...
def employer_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(
                request, 'Registration completed - login via the form below'
            )
...
def jobseeker_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(
                request, 'Registration done - login via the form below'
            )
            return redirect('login')
    else:
        form = UserRegisterForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                messages.success(request, 'The username and password are not correct')
                return redirect('login')
    form = UserLoginForm()
...
def search(request):
    ...
    for i in resaults:
        get_days = now - i.created_date
        if get_days.days > 1:
            i.created_date = f'{get_days.days} days ago'
